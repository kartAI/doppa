...
@inject
def national_scale_spatial_join_databricks_partitioned_16_nodes(
    ...
    configured buildings dataset size executed on Azure Databricks with a 16-worker
    cluster, using the partitioned join strategy (Sedona KDB-tree spatial partitioner
    with the index built on the right side). The dataset size is pulled from DI inside
