...
#
# Join strategy: default. No `broadcast()` hint and no Sedona partitioner
# configuration (`sedona.global.index`, `sedona.join.gridtype`,
# `sedona.join.indexbuildside`). Spark's cost-based optimizer picks the plan;
# for ST_Intersects this typically falls back to `SortMergeJoin`. This variant
# is the apples-to-apples reference point against which the `broadcast` and
# `partitioned` strategies are compared. Repartitioning to cluster parallelism
# is kept (parallelism control, not a join-strategy hint) so the only axis
# that differs between variants is the strategy itself.
#
# Notes:
